...
public class PuzzleCollection
{
	...
	*/
	public void readPuzzle(String a_pathName) throws IOException
	{
		...
		if (!delimiterFound) 
			PuzzlePlayerApplet.errorMessage("The file " + a_pathName + " " +" does not contain a puzzle separetor (---).");
	...
	private void parsePuzzle(String a_text)
	{
		...
		while (input.hasNext())
		{
			...
			else if (line.contains(","))
			{
				...
				PuzzlePlayerApplet.errorMessage("The line: " +  line + "\n" +
		...
		if (!idFound || !titleFound || !wordsFound || charGrid.size() == 0)
			PuzzlePlayerApplet.errorMessage("The text: \n" + a_text + "\n" +
		...
		for (int row = 0; row < gridHeight; row++)
		{
			...
			{
				PuzzlePlayerApplet.errorMessage("Length of the rows are not equal");
			}
	...
	public static int nextIndex()
	{
		if (nextIndex < allPuzzles.size()){
			...
			PuzzlePlayerApplet.errorMessage("There is no more games!");
		return 0;
	}
